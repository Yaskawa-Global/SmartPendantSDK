...
The constructor also supports additional parametes for the IP address of the API server (Smart Pendant) and TCP port, but that can be omitted for the common cases where the application is running on the pendant or on a desktop PC connecting to the Desktop Smart Pendant app.  
